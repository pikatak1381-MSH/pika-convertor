...
import { logCalculators } from "./logarithm.data"
...
const LogarithmCalculator = () => {
  ...
  type PercentageId = (typeof logCalculators)[number]["id"]

  const [activeId, setActiveId] = useState<PercentageId>(logCalculators[0].id)
  const [values, setValues] = useState<number[]>([])

  const calculator = useMemo(() => logCalculators.find((c) => c.id === activeId)!, [activeId])
